...
def make_vocab(num_top=20000):
    """
    create vocabulary for data
    :param num_top: top-k frequency vocab of answers
    ...
    for dataType in data_train_val:
        ...
        for subtype in subtypes:
            ...
            for question in questions:
                ...
                words = [w.replace("'", "").strip() for w in words if len(w.strip()) > 0]  # so e.g. don't -> dont
                qstSet.update(words)
...
def load_vocab(ansFilename="vocab_answers.txt", qstFilename="vocab_questions.txt"):
    """
